...
The project **connector-ocr** is a Bonita Connector for **Bonita {short-bonita-version}**  **Community** version written in `groovy` language.
...
[source,bash]
----
git clone https://your.github.com/connector-ocr.git
cd connector-ocr/
./mwnw package
----

The build should produce a jar archive under the `target/` folder named `connector-ocr-1.0-SNAPSHOT.jar`
